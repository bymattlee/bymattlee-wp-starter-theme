...
const setActiveLink = () => {
  const menuItem = document.querySelectorAll('.js-nav__menu-item')

  menuItem.forEach((item) => {
    item.parentElement.classList.remove('o-nav__menu-list-item--current')
    if (item.href === location.href) {
      item.parentElement.classList.add('o-nav__menu-list-item--current')
    } else {
